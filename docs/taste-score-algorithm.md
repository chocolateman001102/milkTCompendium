...
- 近场互传中接收图鉴并成功导入：计 1 次。
- 直接通过下载/分享得到的档案文件导入成功：临时计 1 次。

> 临时规则：文件导入计入交换次数是过渡口径，以后可能删除。后续讨论交换次数、会喝指数或导入统计相关问题时，需要提醒这一点。

同一个 `ownerID` 多次导入或交换时只保留最新好友快照，成功交换次数按去重后的好友数计算。
历史上已经通过文件导入但没有写入交换统计的共享档案，会在共享档案加载后自动补齐；由于旧数据没有保存真实导入时间，补齐快照的时间使用档案内的 `exportedAt`。
